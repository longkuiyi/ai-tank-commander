...
const HUD: React.FC<Props> = ({ state, onRestart }) => {
  ...
  const [activeTab, setActiveTab] = useState<'COMMANDS' | 'SCOREBOARD'>('COMMANDS');
      ...
      {state.isGameOver && (
        <div className="fixed inset-0 pointer-events-auto bg-black/95 flex items-center justify-center z-[100] p-4">
          <div className="bg-slate-950 border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl max-w-2xl w-full">
            <h1 className={`text-4xl md:text-8xl font-black mb-8 italic ${state.winner === Team.ALLY ? 'text-emerald-400' : 'text-red-500'}`}>
              {state.winner === Team.ALLY ? '战役胜利' : '战役失利'}
            </h1>
            
            <div className="bg-white/5 rounded-3xl p-6 mb-8 text-left max-h-[300px] overflow-y-auto">
               <h3 className="text-xs font-black uppercase text-blue-400 mb-4 border-b border-white/10 pb-2">战后统计报告</h3>
               <table className="w-full text-sm font-mono">
                 <tbody className="divide-y divide-white/5">
                   {allParticipants.slice(0, 5).map(t => (
                     <tr key={t.id}>
                       <td className="py-2 text-white/80">{t.nickname}</td>
                       <td className="py-2 text-right">
                         <span className="text-red-400 font-bold">{t.kills}K</span> / <span className="text-blue-300">{t.assists}A</span>
                       </td>
                       <td className="py-2 text-right text-emerald-400 font-black">{Math.floor(t.score)}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
              ...
              <div className="text-center min-w-[150px]">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-32 h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                    <div className={`h-full transition-all duration-700 ${state.player.health > 40 ? 'bg-blue-500' : 'bg-red-500'}`} style={{ width: `${state.player.health}%` }} />
                  </div>
